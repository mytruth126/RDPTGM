...
function [Leader_score,Leader_pos,Convergence_curve,Bestpos_curve]=WOA(SearchAgents_no,Max_iteration,lb,ub,dim,fobj)
...
while t<Max_iteration
    for i=1:size(Positions,1)    %判断该代鲸鱼的适应度值，并更新最优得分和最优位置
        ...
        fitness=fobj(Positions(i,:)); %注fobj([矩阵]或单个元素值）可直接调用公式，对该1行矩阵内数据代入计算
